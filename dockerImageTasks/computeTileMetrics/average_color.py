...
# imagePath/getTile Source;  I can feed it a file and it works fine
# http://some/long/:api/v1/item/jdklafjdklasjfdas
# so would have to use girder_client..

### Image assumes it's 3 colors
...
def average_color(imagePath, magnification=None):
    ...
    for tile in source.tileIterator(
        ...
        print('x: %d  y: %d  w: %d  h: %d  mag: %r  color: %g %g %g' % (
            tile['x'], tile['y'], tile['width'], tile['height'],
            tile['magnification'], mean, mean, mean))
    mean = numpy.average(tileMeans, axis=0, weights=tileWeights)
    print('Average color: %g %g %g' % (mean, mean, mean))
    return mean
